...
import { GetTripRouteForDropdownList } from 'src/app/helper/common/shared-types';
...
import { GetOpmForDropdownList, GetTripDestinationForDropdownList, GetTripStatusForDropdownList, OPM, TripDestination, TripRoute, TripStatus } from './../../../../../helper/common/shared-types';
import { DropdownItem } from './../../../../../helper/models/common/dropdown/dropdown-item.model';

@Component({
  selector: 'app-report-trip-milage',
  templateUrl: './report-trip-milage.component.html',
  styleUrls: ['./report-trip-milage.component.css']
})
export class ReportTripMilageComponent implements OnInit, OnDestroy {
  grid: GridComponent;
    ...
    this.gridFilterSubscription = this.gridToolbarService.getGridFilter()
      .subscribe(
        (show: boolean) => {
          ...
          this.reportService.fetchTripMilageGridData(this.state, this.searchQuery);
        }
      );
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    this.state = state;
    this.reportService.fetchTripMilageGridData(state, this.searchQuery);
  }
