...
@Component({
  selector: 'app-trip-booking-vip-list',
  templateUrl: './trip-booking-vip-list.component.html',
  styleUrls: ['./trip-booking-vip-list.component.css']
})
export class TripBookingVipListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.refreshScreenSubscription = this.utilityService.refreshData.subscribe({
      next: (flag: boolean) => {
        if (flag) {
          this.tripService.fetchGridData(this.state, this.searchQuery, TripType.VIP);
        }
      },
      ...
      .subscribe(
        (pageSize: number) => {
          ...
          this.tripService.fetchGridData(
            this.state, this.searchQuery, TripType.VIP);
        }
      );
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    this.state = state;
    this.tripService.fetchGridData(
      state, this.searchQuery, TripType.VIP);
  }
